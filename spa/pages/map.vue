  ...
  <v-row justify="center">
    ...
    >
      <v-list dense>
        ...
        <v-list-item v-if="!$getShowNodeInfo()" @click="$setShowNodeInfo(true)">
          <v-list-item-icon><v-icon>mdi-label</v-icon></v-list-item-icon>
          <v-list-item-content>
            <v-list-item-title>IPアドレスを表示</v-list-item-title>
          </v-list-item-content>
        </v-list-item>
        <v-list-item v-if="$getShowNodeInfo()" @click="$setShowNodeInfo(false)">
          <v-list-item-icon><v-icon>mdi-label-off</v-icon></v-list-item-icon>
          <v-list-item-content>
            <v-list-item-title>IPアドレスを非表示</v-list-item-title>
          </v-list-item-content>
        </v-list-item>
      </v-list>
    </v-menu>
    ...
    <v-dialog v-model="memoDialog" persistent max-width="50vw">
      <v-card>
        <v-card-title>
          <span class="headline">メモ</span>
        </v-card-title>
        <v-alert v-model="saveMemoError" color="error" dense dismissible>
          メモの保存に失敗しました
        </v-alert>
        <v-card-text>
          <v-textarea v-model="memo.Memo" label="メモ"></v-textarea>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" @click="saveMemo">
            <v-icon>mdi-content-save</v-icon>
            保存
          </v-btn>
          <v-btn color="normal" @click="memoDialog = false">
            <v-icon>mdi-cancel</v-icon>
            キャンセル
          </v-btn>
        </v-card-actions>
...
export default {
  ...
  methods: {
    ...
    async showMemo() {
      this.saveMemoError = false
      this.memo = await this.$axios.$get('/api/node/memo/' + this.editNode.ID)
      this.memoDialog = true
    },
    async saveMemo() {
      this.saveMemoError = false
      const r = await this.$axios.$post('/api/node/memo', this.memo)
      if (!r) {
        this.saveMemoError = true
      } else {
